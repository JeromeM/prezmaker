...
pub fn list_templates(content_type: &str) -> Result<Vec<ContentTemplate>, String> {
    ...
    if let Some(html_body) = crate::default_templates_html::get_default_html(content_type) {
        ...
        let needs_update = if let Some(existing) = templates.iter().find(|t| t.name == "default-html") {
            ...
            existing.body.contains("{{p:{{field") || existing.body.contains("{{field:Date de sortie:") || existing.body.contains("Voir la fiche") || existing.body.contains("{{ratings_table}}") || existing.body.contains("linear-gradient") || existing.body.contains("separator-red")
        } else {
            true
        };
        if needs_update {
